...
async function postWithOptionalImage(path, payloadCourse, imageFile, method = 'POST') {
  // If imageFile present, send multipart/form-data with course JSON and image
  if (imageFile) {
    const fd = new FormData()
    fd.append('course', JSON.stringify(payloadCourse))
    ...
    try {
      return await (method === 'POST' ? apiPost(path, { course: payloadCourse }, { includeCredentials: true }) : apiPut(path, { course: payloadCourse }, { includeCredentials: true }))
    } catch (err) {
      // fallback to fetch absolute
      const res = await fetch('http://localhost:3000' + path.replace(/^/,'/'), { method, credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ course: payloadCourse }) })
      if (res.ok) return res.json()
